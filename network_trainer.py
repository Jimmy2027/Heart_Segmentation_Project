    ...
    for whichmodel in whichmodels:
        ...
        for layers in layers_arr:
            ...
            for perc_index, perc in enumerate(data_percs):
                for patient_perc in slice_percs:
                    for split_number in seeds:
                        ...
                        x_train, y_train= methods.get_patient_perc_split(input, labels, train_pats, patient_perc)
                        x_test, y_test= methods.get_patient_perc_split(input, labels, test_pats, 1)
                        x_val, y_val= methods.get_patient_perc_split(input, labels, val_pats, 1)

                        input_size = x_train.shape[1:4]

                        print("****************************************************************************************************************")
                        print("EXPERIMENT:", layers, "layers,", perc * 100, "% total data,", patient_perc * 100, "% per pat,",
